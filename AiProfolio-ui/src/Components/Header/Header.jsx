...
function Header({header, data, images}){
    ...
    return (
        <div id="header" style={{...header?.background, background : backgroundImage ? `linear-gradient(
            ...
            <h1 id="header-title" style={{...header?.title, textShadow : backgroundImage ? "#FFF 1px 0px 8px;" : ""}}> {data?.user?.name} </h1>
            <p id="header-subtitle" style={{...header?.subtitle, textShadow : backgroundImage ? "#FFF 1px 0px 5px;" : ""}}> {data?.jobAspiration} </p>
        </div> 
    )
}
